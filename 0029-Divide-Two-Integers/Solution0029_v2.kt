 ...
 */
class Solution0029_v2 {
    fun divide(dividend: Int, divisor: Int): Int {
        assert(divisor != 0)
        if (dividend == 0) return 0
        if (divisor == 1) return dividend
        if (divisor == -1) return if (dividend == Int.MIN_VALUE) Int.MAX_VALUE else -dividend
        val a = dividend.toLong().absoluteValue
        val b = divisor.toLong().absoluteValue
...
/*
执行用时：180 ms, 在所有 Kotlin 提交中击败了60.00%的用户
内存消耗：32.5 MB, 在所有 Kotlin 提交中击败了80.00%的用户
 */